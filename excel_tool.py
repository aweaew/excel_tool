...
class ExcelToolApp:
    def __init__(self, root):
        ...
        self.tree.tag_configure('odd', background='white'); self.tree.tag_configure('even', background='#f2f5f9')
        ...
        frame_preview.grid_columnconfigure(0, weight=1); frame_preview.grid_rowconfigure(0, weight=1)
    ...
    def init_engine_panel(self):
        frame_eng = tk.LabelFrame(self.root, text="⚙️ 引擎配置 (如遇字体变化请切换引擎)", padx=10, pady=2, bg="#F0F8FF")
    ...
    def init_merge_tab(self):
        ...
        tk.Label(frame_top, text="1. 选择回收文件夹:", font=("Microsoft YaHei", 10, "bold")).grid(row=0, column=0, sticky="w")
        ...
        self.lbl_template = tk.Label(frame_top, text="当前模板: [未选择]", fg="gray"); self.lbl_template.grid(row=3, column=1, columnspan=2, sticky="w", padx=10)
        ...
        frame_list = tk.LabelFrame(frame, text="📄 文件列表", padx=10, pady=5); frame_list.pack(fill="both", expand=True, padx=20, pady=5)
        ...
        self.file_tree.column("idx", width=40); self.file_tree.heading("idx", text="序号")
        self.file_tree.column("filename", width=300); self.file_tree.heading("filename", text="文件名")
        self.file_tree.column("rows", width=100); self.file_tree.heading("rows", text="行数")
        self.file_tree.column("size", width=100); self.file_tree.heading("size", text="大小")
    ...
    def load_preview(self, file_path):
        ...
        try:
            ...
            for i in range(1, max_col+1):
                ...
                self.tree.column(str(i), width=100, anchor='w'); self.tree.heading(str(i), text=c)
            ...
            wb.close()
        except Exception as e: self.log(f"预览失败: {e}", "ERROR")
    ...
    def sanitize_file(self, file_path):
        ...
        self.log(f"生成分析缓存 (引擎: {prog_id})...", "INFO")
        app = None
        try:
    ...
    def process_split(self):
        ...
        try:
            count = 0
            if mode == "fast": count = self.run_fast_split(f, start_row, col_idx, out_dir, ts)
            else: count = self.run_perfect_split(f, start_row, col_idx, out_dir, ts)
            
            # [V34新增] 弹窗询问是否打开文件夹
            self.ask_open_folder(out_dir, f"拆分任务全部完成！\n共生成 {count} 个文件。")
    ...
    def run_perfect_split(self, original_file, start_row, col_idx, output_dir, timestamp):
        prog_id = self.get_active_app_name()
        if not prog_id: self.log("未找到引擎", "ERROR"); return
        self.log(f">>> 启动完美模式 (引擎: {prog_id})...", "ENGINE")
        shadow_file = self.sanitize_file(original_file)
        temp_made = (shadow_file != original_file)
        wb_scan = openpyxl.load_workbook(shadow_file, read_only=True, data_only=True)
        ...
        wb_scan.close()
        unique_targets = set(row_data_map.values())
        ...
        try:
            ...
            for idx, target_val in enumerate(unique_targets):
                ...
                self.log(f"[{idx+1}/{len(unique_targets)}] 处理: {t_file}", "INFO")
                shutil.copy2(original_file, t_path)
                wb = app.Workbooks.Open(os.path.abspath(t_path)); ws = wb.ActiveSheet
                
                # 防爆：切除底部空行
                if real_max_row < full_max:
                    try: ws.Range(f"A{real_max_row+1}:A{full_max}").EntireRow.Delete()
                    except: pass
                ...
                for r in range(real_max_row, start_row - 1, -1):
                    ...
                    if not owner or owner != target_val:
    ...
    def scan_merge_folder(self):
        ...
        self.merge_files_cache = []; self.current_template = None; self.lbl_template.config(text="当前模板: [未选择]", fg="gray")
        files = glob.glob(os.path.join(folder, "*.xlsx"))
        files = [f for f in files if "汇总" not in os.path.basename(f) and not os.path.basename(f).startswith("~$")]
        if not files: self.log("未找到文件", "WARN"); return
        ...
        for idx, f in enumerate(files):
            try:
                ...
                for i, r in enumerate(ws.iter_rows(min_col=1, max_col=1, values_only=True)):
                    ...
                    if i+1>=start_r and r[0]: rows_est += 1 if isinstance(rows_est, int) else 0
                wb.close()
    ...
    def on_file_list_click(self, event):
        ...
        idx = int(self.file_tree.item(item_id, 'values')[0]) - 1
    ...
    def process_merge(self):
        ...
        try:
            ...
            if HAS_WIN32:
                clean_tmpl = self.sanitize_file(templ)
                shutil.copy2(clean_tmpl, save_path)
                if clean_tmpl != templ: shutil.rmtree(os.path.dirname(clean_tmpl))
            ...
            # [V34新增] 弹窗询问
            self.ask_open_folder(os.path.dirname(save_path), f"合并完成！共 {cnt} 行数据。\n文件: {save_name}")
            
        except Exception as e: self.log(f"合并错误: {e}", "ERROR")

    # --- 强力清理 ---
    def process_clean_save(self):
        f = self.entry_file_path.get()
        if not f: return
        self.clear_log(); self.log("开始清洗...", "INFO")
        if HAS_WIN32: safe_f = self.sanitize_file(f); temp_mk = (safe_f != f)
        else: safe_f = f; temp_mk = False
        try:
            wb = openpyxl.load_workbook(safe_f); ws = wb.active
            for r in range(ws.max_row, 0, -1):
                if all(not ws.cell(row=r, column=c).value for c in range(1, ws.max_column+1)): ws.delete_rows(r)
            ts = datetime.datetime.now().strftime("%H%M%S")
            nm = f"{os.path.splitext(os.path.basename(f))[0]}_清洗_{ts}.xlsx"
            out_path = os.path.join(os.path.dirname(f), nm)
            wb.save(out_path)
            if temp_mk: shutil.rmtree(os.path.dirname(safe_f))
            
            # [V34新增] 弹窗询问
            self.ask_open_folder(os.path.dirname(out_path), f"清洗完成: {nm}")
    ...
    def generate_analysis_report(self):
        ...
        try:
            ...
            for i, r in enumerate(ws.iter_rows(values_only=True)):
                ...
                if i+1 >= start_row:
                    val = r[col_idx-1] if col_idx-1 < len(r) else None
                    if val: vals.append(str(val).strip())
            wb.close()
            counter = Counter(vals)
            rep = [f"文件: {os.path.basename(f)}", f"总行: {total}", f"数据行: {data_cnt}", "-"*30, "【分类预览】"]
